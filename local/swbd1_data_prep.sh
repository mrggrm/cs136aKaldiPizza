...
if [ ! -d $SWBD_DIR/train/transcript/swbd_train/ ]; then ### Check for the transcriptions directory and download if necessary
  ( 
...
[ ! -f $SWBD_DIR/dict/sw-ms98-dict_patched.text ] && \
...
}' $SWBD_DIR/train/transcript/swbd_train/*-trans.text  > $dir/transcripts1.txt
...
# case insensitive
$SWBD_DIR/../local/swbd1_map_words.pl -f 2- $dir/transcripts2.txt  > $dir/text ### Map the words of the transcriptions and write to text directory

# format acronyms in text
### Format all of the acronyms in the transcriptions and move to the text directory
python $SWBD_DIR/../local/map_acronyms_transcripts.py -i $dir/text -o $dir/text_map \
-M data/dict/acronyms.map  
...
utils/fix_data_dir.sh data/train/swbd
